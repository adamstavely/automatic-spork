...
function copyAndMergeDeep(base, override) {
  const output = __spreadValues({}, base);
...
function strokeMatches(userStroke, character, strokeNum, options = {}) {
  ...
  for (let i = 0; i < laterStrokes.length; i++) {
    const {
      isMatch: isMatch2,
      avgDist: avgDist2
    } = getMatchData(points, laterStrokes[i], __spreadProps(__spreadValues({}, options), {
  ...
  if (closestMatchDist < avgDist) {
    ...
    const {
      isMatch: isMatch2,
      meta: meta2
    } = getMatchData(points, strokes[strokeNum], __spreadProps(__spreadValues({}, options), {
...
var getMatchData = (points, stroke, options) => {
  ...
  if (checkBackwards && !isMatch) {
    const backwardsMatchData = getMatchData([...points].reverse(), stroke, __spreadProps(__spreadValues({}, options), {
      checkBackwards: false
    }));
...
var Quiz = class {
  ...
  _handleSuccess(meta) {
    ...
    onCorrectStroke === null || onCorrectStroke === void 0 ? void 0 : onCorrectStroke(__spreadValues({}, this._getStrokeData({
      isCorrect: true,
      meta
    })));
    this.nextStroke();
...
var HanziWriterRenderer = class {
  ...
  render(props) {
    ...
    for (const userStrokeId in userStrokes) {
      ...
      const userStrokeProps = __spreadValues({
...
var HanziWriter = class _HanziWriter {
  ...
  static loadCharacterData(character, options = {}) {
    const loadingManager = (() => {
      ...
      return new LoadingManager(__spreadValues(__spreadValues({}, defaultOptions), options));
    })();
  ...
  quiz(quizOptions = {}) {
    return this._withData(() => __async(this, null, function* () {
      if (this._character && this._renderState && this._positioner) {
        ...
        this._options = __spreadValues(__spreadValues({}, this._options), quizOptions);
  ...
  getCharacterData() {
    return __async(this, null, function* () {
      ...
      const character = yield this._withData(() => this._character);
      return character;
    });
  }
  ...
  _fillWidthAndHeight(options) {
    const filledOpts = __spreadValues({}, options);
